...
class MMPNERProcessor(object):
    ...
    def load_from_file(self, mode="train", sample_ratio=1.0):
        ...
        aux_imgs = torch.load(aux_path)
        # aux_imgs = torch.load(root/CHB/HVPNeT-main/data/NER_data/twitter2015_aux_images/test)
...
class MMREDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.img_path is not None:
            ...
            return input_ids, attention_mask, torch.tensor(re_label), image, aux_imgs
        # return input_ids, token_type_ids, attention_mask, torch.tensor(re_label)
        return input_ids, attention_mask, torch.tensor(re_label)
    ...
    def __getitem__(self, idx):
        ...
        if self.img_path is not None:
            ...
            if self.aux_img_path is not None:
                ...
                return torch.tensor(input_ids),  torch.tensor(attention_mask), torch.tensor(labels), image, aux_imgs
